#!/bin/bash
#
# validator-launchd-wrapper.sh: macOS supervisor for ABCI + CometBFT.
#
# Equivalent to the systemd service pair:
#   ensoul-abci.service    (Restart=always, ExecStopPost=auto-upgrade.sh)
#   ensoul-cometbft.service (Restart=always, Requires=ensoul-abci.service)
#
...
#
# This script is the ProgramArguments target for the launchd plist.
# launchd's KeepAlive restarts THIS SCRIPT if it crashes. The internal
# restart loop handles the ABCI/CometBFT lifecycle, including the
# auto-upgrade gap between exit and restart.
#
# Compatible with macOS system bash (3.2). Uses a poll loop instead of
# wait -n to detect either process exiting (2s detection latency).
#
# Usage:
#   Managed by launchd via dev.ensoul.validator.plist. Can also be run manually:
#   ./scripts/validator-launchd-wrapper.sh
...
# CometBFT via Cosmovisor
COSMOVISOR="$HOME/go/bin/cosmovisor"
...
stop_cometbft() {
    ...
    # (Cosmovisor forks a child cometbft with a different PID)
    local port_pid
...
while true; do
    ...
    # (macOS system bash). Detection latency is at most 2s.
    while true; do
        if ! kill -0 "$ABCI_PID" 2>/dev/null; then
    ...
    # Regardless of which exited, stop CometBFT first, then ABCI.
    stop_cometbft
    stop_abci

    # ── Step 5: Run auto-upgrade.sh (ExecStopPost equivalent) ──
    # Idempotent: no-op if no upgrade-info.json exists.
